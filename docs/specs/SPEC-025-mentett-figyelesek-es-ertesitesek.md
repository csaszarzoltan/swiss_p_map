---
id: FEAT-025
title: Mentett Figyelési Zónák és Építési Értesítések
status: SPEC_READY
version: 1
risk: high
owner: product-owner
...
# FEAT-025: Mentett Figyelési Zónák és Építési Értesítések
...
- Kapcsolódó brief: `BRIEF-025` (`docs/briefs/BRIEF-025-mentett-figyelesek-es-ertesitesek.md`)
...
- REQ-001 [MUST]: A rendszer a mentett figyelési zónák és építési értesítések hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a mentett figyelési zónák és építési értesítések hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a mentett figyelési zónák és építési értesítések hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a mentett figyelési zónák és építési értesítések hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a mentett figyelési zónák és építési értesítések hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `POST /api/v1/watch-zones`; `GET /api/v1/watch-zones`; `PATCH /api/v1/watch-zones/{id}`; `DELETE /api/v1/watch-zones/{id}`
