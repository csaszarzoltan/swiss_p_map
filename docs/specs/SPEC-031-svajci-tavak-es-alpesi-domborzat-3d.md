...
- Kapcsolódó brief: `BRIEF-031` (`docs/briefs/BRIEF-031-svajci-tavak-es-alpesi-domborzat-3d.md`)
...
- REQ-001 [MUST]: A rendszer a svájci tavak és alpesi domborzati magasságok 3d-ben hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a svájci tavak és alpesi domborzati magasságok 3d-ben hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a svájci tavak és alpesi domborzati magasságok 3d-ben hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a svájci tavak és alpesi domborzati magasságok 3d-ben hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a svájci tavak és alpesi domborzati magasságok 3d-ben hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/map/lakes`; `GET /api/v1/map/elevation/cantons`
