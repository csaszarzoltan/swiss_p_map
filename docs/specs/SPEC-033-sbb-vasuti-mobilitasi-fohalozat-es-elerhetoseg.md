---
id: FEAT-033
title: SBB Vasúti Mobilitási Főhálózat és Állomási Elérhetőség
status: SPEC_READY
version: 1
risk: medium
owner: product-owner
...
# FEAT-033: SBB Vasúti Mobilitási Főhálózat és Állomási Elérhetőség
...
- Kapcsolódó brief: `BRIEF-033` (`docs/briefs/BRIEF-033-sbb-vasuti-mobilitasi-fohalozat-es-elerhetoseg.md`)
...
- REQ-001 [MUST]: A rendszer a sbb vasúti mobilitási főhálózat és állomási elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a sbb vasúti mobilitási főhálózat és állomási elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a sbb vasúti mobilitási főhálózat és állomási elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a sbb vasúti mobilitási főhálózat és állomási elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a sbb vasúti mobilitási főhálózat és állomási elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/mobility/railways`; `GET /api/v1/mobility/access?lat={lat}&lon={lon}`
