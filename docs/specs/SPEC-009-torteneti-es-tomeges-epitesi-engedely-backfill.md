---
id: FEAT-009
title: Történeti és Tömeges Építési Engedély Backfill
status: SPEC_READY
version: 1
risk: medium
owner: product-owner
...
# FEAT-009: Történeti és Tömeges Építési Engedély Backfill
...
- Kapcsolódó brief: `BRIEF-009` (`docs/briefs/BRIEF-009-torteneti-es-tomeges-epitesi-engedely-backfill.md`)
- Kapcsolódó research: `docs/research/2026-08-27-ogd-backfill.md`
- Kapcsolódó ADR: `ADR-009` (`docs/decisions/ADR-009-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `src/services/ogd_service.py; src/db/planning_repo.py; src/main.py`
...
- REQ-001 [MUST]: A rendszer a történeti és tömeges építési engedély backfill hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a történeti és tömeges építési engedély backfill hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a történeti és tömeges építési engedély backfill hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a történeti és tömeges építési engedély backfill hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a történeti és tömeges építési engedély backfill hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: `POST /api/v1/planning/backfill`
