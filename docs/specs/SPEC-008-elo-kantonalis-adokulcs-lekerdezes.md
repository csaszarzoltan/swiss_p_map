---
id: FEAT-008
title: Élő Kantonális Adókulcs Lekérdezés
status: SPEC_READY
version: 1
risk: medium
owner: product-owner
...
# FEAT-008: Élő Kantonális Adókulcs Lekérdezés
...
- Kapcsolódó brief: `BRIEF-008` (`docs/briefs/BRIEF-008-elo-kantonalis-adokulcs-lekerdezes.md`)
- Kapcsolódó research: `docs/research/2026-08-27-zh-steuerfuss-live.md`
- Kapcsolódó ADR: `ADR-008` (`docs/decisions/ADR-008-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `src/services/place_service.py; tests/unit/test_place_zh_steuerfuss.py`
...
- REQ-001 [MUST]: A rendszer a élő kantonális adókulcs lekérdezés hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a élő kantonális adókulcs lekérdezés hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a élő kantonális adókulcs lekérdezés hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a élő kantonális adókulcs lekérdezés hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a élő kantonális adókulcs lekérdezés hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: `GET /api/v1/place/{postcode}?live=true`
