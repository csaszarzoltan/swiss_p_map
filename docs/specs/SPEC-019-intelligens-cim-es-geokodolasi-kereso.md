---
id: FEAT-019
title: Intelligens Cím- és Geokódolási Kereső
status: SPEC_READY
version: 1
risk: low
owner: product-owner
...
# FEAT-019: Intelligens Cím- és Geokódolási Kereső
...
- Kapcsolódó brief: `BRIEF-019` (`docs/briefs/BRIEF-019-intelligens-cim-es-geokodolasi-kereso.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `frontend/src/app/SearchPanel.tsx; src/services/swisstopo_service.py`
...
- REQ-001 [MUST]: A rendszer a intelligens cím- és geokódolási kereső hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a intelligens cím- és geokódolási kereső hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a intelligens cím- és geokódolási kereső hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a intelligens cím- és geokódolási kereső hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a intelligens cím- és geokódolási kereső hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: Tervezett: `GET /api/v1/geocode/search?q={text}&limit={1..10}`
