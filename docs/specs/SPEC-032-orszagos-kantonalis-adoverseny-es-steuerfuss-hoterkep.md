---
id: FEAT-032
title: Országos Kantonális Adóverseny és Steuerfuss Hőtérkép
status: SPEC_READY
version: 1
risk: medium
owner: product-owner
...
# FEAT-032: Országos Kantonális Adóverseny és Steuerfuss Hőtérkép
...
- Kapcsolódó brief: `BRIEF-032` (`docs/briefs/BRIEF-032-orszagos-kantonalis-adoverseny-es-steuerfuss-hoterkep.md`)
...
- REQ-001 [MUST]: A rendszer a országos kantonális adóverseny és steuerfuss hőtérkép hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a országos kantonális adóverseny és steuerfuss hőtérkép hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a országos kantonális adóverseny és steuerfuss hőtérkép hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a országos kantonális adóverseny és steuerfuss hőtérkép hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a országos kantonális adóverseny és steuerfuss hőtérkép hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/tax/cantons`; `GET /api/v1/tax/{canton_code}`; `GET /api/v1/tax/compare`
