...
id: SPEC-060
feature: FEAT-060
title: Böngészős Web Push és Figyelési Zóna Riasztási Rendszer
status: SPEC_READY
...
brief: BRIEF-060
---
# SPEC-060: Böngészős Web Push és Figyelési Zóna Riasztási Rendszer
## 1. Cím és metaadatok
`SPEC-060`, `BRIEF-060`, verzió 1, SPEC_READY, implementáció PENDING_DEV.
## 2. Cél és kontextus
Resident-first civic UX, élő OGD integráció vagy hozzájárulás-alapú értesítés biztosítása ellenőrizhető forrással.
## 3. Funkcionális követelmények
- REQ-060-001 [MUST]: A feature típusos, determinisztikus sikeres választ vagy UI-eredményt ad.
- REQ-060-002 [MUST]: Minden külső adat source, fetched_at és trust_state metaadatot tartalmaz.
- REQ-060-003 [MUST]: Loading, success, empty, stale/source_pending és error állapot támogatott.
- REQ-060-004 [MUST NOT]: Duplikált értesítés, hamis élő adat vagy hozzájárulás nélküli kézbesítés tilos.
- REQ-060-005 [ALWAYS]: A fallback nem jelenhet meg official_measurement státusszal.
- REQ-060-006 [CONCURRENCY]: Idempotency/deduplikáció és legutóbbi kérés nyer szabály kötelező.
## 4. Nem-funkcionális követelmények
- NFR-060-001 [PERFORMANCE]: UI feedback 100 ms; API timeout 10 s; connector cache a provider ciklusához kötött.
- NFR-060-002 [ACCESSIBILITY]: WCAG 2.1 AA, billentyűzet, aria-live, grafikonhoz szöveges alternatíva.
- NFR-060-003 [PRIVACY]: nDSG/GDPR minimalizálás; email/push token titkos vagy hash-elt tárolása.
## 5. Elfogadási kritériumok
### AC-060-001: Happy path
...
### AC-060-003: Hibás vagy ismételt kérés
...
`POST /api/v1/push/subscribe; POST /api/v1/push/watch-alert`. JSON response source/trust/cache metaadatokkal; írások idempotency mezővel.
...
## 11. Nemzetköziesítés
DE/EN/FR/IT kulcsnévtér: `phase3.feature060.*`.
## 12. Akadálymentesség
...
- REQ-060-001 -> AC-060-001 -> unit + API/E2E.
- REQ-060-002 -> AC-060-001 -> contract test.
- REQ-060-003 -> AC-060-001..003 -> UI state test.
- REQ-060-004 -> AC-060-003 -> negative/dedup test.
- REQ-060-005 -> AC-060-002 -> fallback test.
- REQ-060-006 -> AC-060-003 -> concurrency/idempotency test.
