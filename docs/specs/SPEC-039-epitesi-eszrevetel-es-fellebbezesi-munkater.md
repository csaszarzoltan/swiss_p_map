---
id: FEAT-039
title: Építési Észrevétel- és Fellebbezési Munkatér
status: SPEC_READY
version: 1
risk: high
owner: product-owner
...
# FEAT-039: Építési Észrevétel- és Fellebbezési Munkatér
...
- Kapcsolódó brief: `BRIEF-039` (`docs/briefs/BRIEF-039-epitesi-eszrevetel-es-fellebbezesi-munkater.md`)
...
- REQ-001 [MUST]: A rendszer a építési észrevétel- és fellebbezési munkatér hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a építési észrevétel- és fellebbezési munkatér hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a építési észrevétel- és fellebbezési munkatér hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a építési észrevétel- és fellebbezési munkatér hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a építési észrevétel- és fellebbezési munkatér hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `POST /api/v1/cases`; `GET/PATCH/DELETE /api/v1/cases/{id}`; `POST /api/v1/cases/{id}/draft`
