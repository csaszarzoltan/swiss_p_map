---
id: FEAT-005
title: Élő Szövetségi Körzeti és Politikai Integráció
status: SPEC_READY
version: 1
risk: medium
owner: product-owner
...
# FEAT-005: Élő Szövetségi Körzeti és Politikai Integráció
...
- Kapcsolódó brief: `BRIEF-005` (`docs/briefs/BRIEF-005-elo-szovetsegi-korzeti-es-politikai-integracio.md`)
- Kapcsolódó research: `docs/research/2026-08-26-politics-live-sources.md`
- Kapcsolódó ADR: `ADR-005` (`docs/decisions/ADR-005-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `src/services/place_service.py; src/services/politics_service.py; src/main.py`
...
- REQ-001 [MUST]: A rendszer a élő szövetségi körzeti és politikai integráció hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a élő szövetségi körzeti és politikai integráció hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a élő szövetségi körzeti és politikai integráció hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a élő szövetségi körzeti és politikai integráció hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a élő szövetségi körzeti és politikai integráció hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: `GET /api/v1/place/{postcode}?live={bool}`; `GET /api/v1/politics/representatives?postcode={PLZ}&live={bool}`
