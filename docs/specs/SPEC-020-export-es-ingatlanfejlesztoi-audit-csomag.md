...
- Kapcsolódó brief: `BRIEF-020` (`docs/briefs/BRIEF-020-export-es-ingatlanfejlesztoi-audit-csomag.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `frontend/src/components/DetailPanel.tsx`
...
- REQ-001 [MUST]: A rendszer a export és ingatlanfejlesztői audit csomag hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a export és ingatlanfejlesztői audit csomag hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a export és ingatlanfejlesztői audit csomag hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a export és ingatlanfejlesztői audit csomag hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a export és ingatlanfejlesztői audit csomag hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: Tervezett: `GET /api/v1/place/{postcode}/export?format=json|csv`; PDF böngészős nyomtatási nézet.
