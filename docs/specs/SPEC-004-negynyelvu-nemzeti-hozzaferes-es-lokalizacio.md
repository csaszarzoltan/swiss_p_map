---
id: FEAT-004
title: Négynyelvű Nemzeti Hozzáférés és Lokalizáció
status: SPEC_READY
version: 1
risk: low
owner: product-owner
...
# FEAT-004: Négynyelvű Nemzeti Hozzáférés és Lokalizáció
...
- Kapcsolódó brief: `BRIEF-004` (`docs/briefs/BRIEF-004-negynyelvu-nemzeti-hozzaferes-es-lokalizacio.md`)
- Kapcsolódó research: `docs/research/2026-08-26-i18n-4lang.md`
- Kapcsolódó ADR: `ADR-004` (`docs/decisions/ADR-004-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `frontend/src/app/[locale]/; frontend/src/i18n/; frontend/messages/`
...
- REQ-001 [MUST]: A rendszer a négynyelvű nemzeti hozzáférés és lokalizáció hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a négynyelvű nemzeti hozzáférés és lokalizáció hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a négynyelvű nemzeti hozzáférés és lokalizáció hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a négynyelvű nemzeti hozzáférés és lokalizáció hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a négynyelvű nemzeti hozzáférés és lokalizáció hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: Nincs új backend végpont; útvonalszerződés: `/{locale}` ahol locale ∈ `{de,en,fr,it}`.
