---
id: FEAT-043
title: Épületenergetika, Felújítási Potenciál és Fűtéscsere
status: SPEC_READY
version: 1
risk: high
owner: product-owner
...
# FEAT-043: Épületenergetika, Felújítási Potenciál és Fűtéscsere
...
- Kapcsolódó brief: `BRIEF-043` (`docs/briefs/BRIEF-043-epuletenergetika-felujitasi-potencial-es-futescsere.md`)
...
- REQ-001 [MUST]: A rendszer a épületenergetika, felújítási potenciál és fűtéscsere hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a épületenergetika, felújítási potenciál és fűtéscsere hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a épületenergetika, felújítási potenciál és fűtéscsere hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a épületenergetika, felújítási potenciál és fűtéscsere hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a épületenergetika, felújítási potenciál és fűtéscsere hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/building-energy/profile?building_id={id}`
