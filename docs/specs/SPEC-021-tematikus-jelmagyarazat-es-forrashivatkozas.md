...
- Kapcsolódó brief: `BRIEF-021` (`docs/briefs/BRIEF-021-tematikus-jelmagyarazat-es-forrashivatkozas.md`)
- Kapcsolódó research: `docs/research/2026-08-28-usability-deep-dive.md`
- Kapcsolódó ADR: `ADR-019` (`docs/decisions/ADR-019-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `frontend/src/components/MapLegend.tsx`
...
- REQ-001 [MUST]: A rendszer a tematikus jelmagyarázat és forráshivatkozás hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a tematikus jelmagyarázat és forráshivatkozás hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a tematikus jelmagyarázat és forráshivatkozás hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a tematikus jelmagyarázat és forráshivatkozás hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a tematikus jelmagyarázat és forráshivatkozás hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: Nincs új backend végpont; forráslink és skála kliensoldali szerződés.
