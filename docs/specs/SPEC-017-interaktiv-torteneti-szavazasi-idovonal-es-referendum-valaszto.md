---
id: FEAT-017
title: Interaktív Történeti Szavazási Idővonal és Referendum Választó
status: SPEC_READY
version: 1
risk: low
owner: product-owner
...
# FEAT-017: Interaktív Történeti Szavazási Idővonal és Referendum Választó
...
- Kapcsolódó brief: `BRIEF-017` (`docs/briefs/BRIEF-017-interaktiv-torteneti-szavazasi-idovonal-es-referendum-valaszto.md`)
- Kapcsolódó research: `docs/research/2026-08-27-referendum-timeline.md`
- Kapcsolódó ADR: `ADR-017` (`docs/decisions/ADR-017-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `src/services/vote_service.py; src/main.py; frontend/src/app/Map3D.tsx`
...
- REQ-001 [MUST]: A rendszer a interaktív történeti szavazási idővonal és referendum választó hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a interaktív történeti szavazási idővonal és referendum választó hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a interaktív történeti szavazási idővonal és referendum választó hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a interaktív történeti szavazási idővonal és referendum választó hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a interaktív történeti szavazási idővonal és referendum választó hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: `GET /api/v1/politics/votes/list`; `GET /api/v1/politics/votes/{proposal_id}`
