---
id: FEAT-018
title: Térbeli Sugár- és Poligon Keresőmotor
status: SPEC_READY
version: 1
risk: medium
owner: product-owner
...
# FEAT-018: Térbeli Sugár- és Poligon Keresőmotor
...
- Kapcsolódó brief: `BRIEF-018` (`docs/briefs/BRIEF-018-terbeli-sugar-es-poligon-keresomotor.md`)
- Kapcsolódó research: `docs/research/2026-08-27-spatial-radius-engine.md`
- Kapcsolódó ADR: `ADR-018` (`docs/decisions/ADR-018-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `src/services/geo_converter.py; src/db/planning_repo.py; src/main.py`
...
- REQ-001 [MUST]: A rendszer a térbeli sugár- és poligon keresőmotor hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a térbeli sugár- és poligon keresőmotor hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a térbeli sugár- és poligon keresőmotor hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a térbeli sugár- és poligon keresőmotor hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a térbeli sugár- és poligon keresőmotor hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: `GET /api/v1/planning/radius?lat={lat}&lon={lon}&radius_m={m}&active_only={bool}`; `GET /api/v1/planning/bbox?min_lat={x}&min_lon={y}&max_lat={x}&max_lon={y}&active_only={bool}`
