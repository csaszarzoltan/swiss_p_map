---
id: FEAT-028
title: Körzetek Összehasonlítása és Statisztikai Elemzés
status: SPEC_READY
version: 1
risk: medium
owner: product-owner
...
# FEAT-028: Körzetek Összehasonlítása és Statisztikai Elemzés
...
- Kapcsolódó brief: `BRIEF-028` (`docs/briefs/BRIEF-028-korzetek-osszehasonlitasa-es-statisztikai-elemzes.md`)
...
- REQ-001 [MUST]: A rendszer a körzetek összehasonlítása és statisztikai elemzés hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a körzetek összehasonlítása és statisztikai elemzés hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a körzetek összehasonlítása és statisztikai elemzés hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a körzetek összehasonlítása és statisztikai elemzés hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a körzetek összehasonlítása és statisztikai elemzés hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `POST /api/v1/compare` 2-5 PLZ azonosítóval
