---
id: FEAT-015
title: Tematikus Térképi Hőtérkép és Rétegek
status: SPEC_READY
version: 1
risk: low
owner: product-owner
...
# FEAT-015: Tematikus Térképi Hőtérkép és Rétegek
...
- Kapcsolódó brief: `BRIEF-015` (`docs/briefs/BRIEF-015-tematikus-terkepi-hoterkep-es-retegek.md`)
- Kapcsolódó research: `docs/research/2026-08-27-thematic-map-layers.md`
- Kapcsolódó ADR: `ADR-015` (`docs/decisions/ADR-015-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `frontend/src/app/Map3D.tsx; frontend/src/components/MapLegend.tsx`
...
- REQ-001 [MUST]: A rendszer a tematikus térképi hőtérkép és rétegek hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a tematikus térképi hőtérkép és rétegek hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a tematikus térképi hőtérkép és rétegek hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a tematikus térképi hőtérkép és rétegek hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a tematikus térképi hőtérkép és rétegek hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: Nincs új backend végpont; a réteg a Vote és Place válaszok alapján működik.
