---
id: FEAT-027
title: Offline PWA és Gyenge Hálózati Mód
status: SPEC_READY
version: 1
risk: high
owner: product-owner
...
# FEAT-027: Offline PWA és Gyenge Hálózati Mód
...
- Kapcsolódó brief: `BRIEF-027` (`docs/briefs/BRIEF-027-offline-pwa-es-gyenge-halozati-mod.md`)
...
- REQ-001 [MUST]: A rendszer a offline pwa és gyenge hálózati mód hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a offline pwa és gyenge hálózati mód hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a offline pwa és gyenge hálózati mód hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a offline pwa és gyenge hálózati mód hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a offline pwa és gyenge hálózati mód hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/offline/snapshot/{postcode}` és verziózott Cache API szerződés
