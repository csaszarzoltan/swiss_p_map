---
id: FEAT-007
title: Részletes Helyi Körzetbővítés és Információs Központ
status: SPEC_READY
version: 1
risk: low
owner: product-owner
...
# FEAT-007: Részletes Helyi Körzetbővítés és Információs Központ
...
- Kapcsolódó brief: `BRIEF-007` (`docs/briefs/BRIEF-007-reszletes-helyi-korzetbovites-es-informacios-kozpont.md`)
- Kapcsolódó research: `docs/research/2026-08-27-ort-expansion-sonnendach-oereb.md`
- Kapcsolódó ADR: `ADR-007` (`docs/decisions/ADR-007-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `src/models/place.py; src/services/place_service.py; frontend/src/components/TopicList.tsx`
...
- REQ-001 [MUST]: A rendszer a részletes helyi körzetbővítés és információs központ hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a részletes helyi körzetbővítés és információs központ hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a részletes helyi körzetbővítés és információs központ hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a részletes helyi körzetbővítés és információs központ hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a részletes helyi körzetbővítés és információs központ hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: `GET /api/v1/place/{postcode}?live={bool}`
