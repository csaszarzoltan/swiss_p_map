---
id: FEAT-035
title: Természeti Veszélyek: Árvíz, Felszíni Lefolyás, Lavina és Földcsuszamlás
status: SPEC_READY
version: 1
risk: high
owner: product-owner
...
# FEAT-035: Természeti Veszélyek: Árvíz, Felszíni Lefolyás, Lavina és Földcsuszamlás
...
- Kapcsolódó brief: `BRIEF-035` (`docs/briefs/BRIEF-035-termeszeti-veszelyek-arviz-lavina-es-foldcsuszamlas.md`)
- Kapcsolódó research: nincs feature-specifikus jóváhagyott kutatás; fejlesztés előtt discovery szükséges
- Kapcsolódó ADR: nincs; tartós provider/adatmodell döntés előtt új ADR kötelező
...
- Implementációs bizonyíték / tervezett érintettség: `tervezett provider service; FastAPI válaszmodell; frontend témakomponens; célzott unit/integration/E2E tesztek`
...
- REQ-001 [MUST]: A rendszer a természeti veszélyek: árvíz, felszíni lefolyás, lavina és földcsuszamlás hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a természeti veszélyek: árvíz, felszíni lefolyás, lavina és földcsuszamlás hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a természeti veszélyek: árvíz, felszíni lefolyás, lavina és földcsuszamlás hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a természeti veszélyek: árvíz, felszíni lefolyás, lavina és földcsuszamlás hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a természeti veszélyek: árvíz, felszíni lefolyás, lavina és földcsuszamlás hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/hazards/layers?canton={code}`; `POST /api/v1/hazards/identify`
- Request: szigorúan típusos, ismeretlen mező elutasítva; koordináták WGS84-ben, időpontok ISO 8601 UTC-ben.
- Response 200: verziózott JSON séma, `data` és mezőszintű `provenance` metaadat.
- Response 400/404/409/422: stabil gépi `code`, `message`, `field` hibaséma; upstream hibánál 503 és `retry_after`, ha nincs hiteles cache.
