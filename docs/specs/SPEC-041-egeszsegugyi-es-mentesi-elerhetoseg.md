---
id: FEAT-041
title: Egészségügyi és Mentési Elérhetőség
status: SPEC_READY
version: 1
risk: high
owner: product-owner
...
# FEAT-041: Egészségügyi és Mentési Elérhetőség
...
- Kapcsolódó brief: `BRIEF-041` (`docs/briefs/BRIEF-041-egeszsegugyi-es-mentesi-elerhetoseg.md`)
...
- REQ-001 [MUST]: A rendszer a egészségügyi és mentési elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a egészségügyi és mentési elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a egészségügyi és mentési elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a egészségügyi és mentési elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a egészségügyi és mentési elérhetőség hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/healthcare/nearby?lat={lat}&lon={lon}&type={enum}`
