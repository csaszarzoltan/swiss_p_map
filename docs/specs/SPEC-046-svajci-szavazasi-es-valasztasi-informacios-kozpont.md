---
id: FEAT-046
title: Svájci Szavazási és Választási Információs Központ
status: SPEC_READY
...
# FEAT-046: Svájci Szavazási és Választási Információs Központ

## 1. Cím és metaadatok
- Specifikáció: `SPEC-046`; kapcsolódó brief: `BRIEF-046`.
...
- REQ-046-001 [MUST]: Érvényes bemenetre strukturált, típusos választ kell adni.
- REQ-046-002 [MUST]: Minden állításhoz forrás, időbélyeg és bizalmi állapot kell.
- REQ-046-003 [MUST]: A UI loading, success, empty, source_pending és error állapotot kezel.
- REQ-046-004 [MUST NOT]: Felmérés nem jelenhet meg eredményként, becslés mérésként vagy generált hír tényként.
- REQ-046-005 [ALWAYS]: Hiba után megmarad a hely, nyelv és korábbi érvényes állapot.
- REQ-046-006 [CONCURRENCY]: Csak a legutóbbi kérés válasza frissítheti az aktív nézetet.

## 4. Nem-funkcionális követelmények
- NFR-046-001 [PERFORMANCE]: 100 ms-on belüli loading visszajelzés és 10 s timeout.
- NFR-046-002 [ACCESSIBILITY]: WCAG 2.1 AA, billentyűzet, látható fókusz és aria-live.
- NFR-046-003 [SECURITY]: Pydantic validáció, HTTPS linkek, HTML-injektálás tiltása.

## 5. Elfogadási kritériumok
### AC-046-001: Sikeres lekérés
...
- `GET /api/v1/votes/proposals; GET /api/v1/votes/proposals/{id}/analysis`
- HTTP 200: verziózható JSON; hibák: 404, 422, kontrollált 503.

## 9. Frontend komponensek és UI interakciók
- `frontend/src/components/VotingCenter.tsx`; interaktív tab/modal, Escape bezárás, „Auf Karte” csak szükség esetén.
...
## 11. Nemzetköziesítés
DE, EN, FR és IT; kulcsnévtér `resident.feature046.*`; felhasználói szöveg nem hardkódolható a végleges UI-ban.
...
- REQ-046-001 -> AC-046-001 -> unit + API + E2E.
- REQ-046-002 -> AC-046-001 -> contract + SourceTrustBadge.
- REQ-046-003 -> AC-046-001..003 -> UI state E2E.
- REQ-046-004 -> AC-046-002 -> negatív forrás-integritási teszt.
- REQ-046-005 -> AC-046-002 -> state recovery.
- REQ-046-006 -> AC-046-002 -> concurrency teszt.
