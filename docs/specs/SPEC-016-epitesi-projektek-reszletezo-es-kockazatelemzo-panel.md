---
id: FEAT-016
title: Építési Projektek Részletező és Kockázatelemző Panel
status: SPEC_READY
version: 1
risk: medium
owner: product-owner
...
# FEAT-016: Építési Projektek Részletező és Kockázatelemző Panel
...
- Kapcsolódó brief: `BRIEF-016` (`docs/briefs/BRIEF-016-epitesi-projektek-reszletezo-es-kockazatelemzo-panel.md`)
- Kapcsolódó research: `docs/research/2026-08-27-baugesuch-deep-inspector.md`
- Kapcsolódó ADR: `ADR-016` (`docs/decisions/ADR-016-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `src/models/planning.py; frontend/src/components/DetailPanel.tsx`
...
- REQ-001 [MUST]: A rendszer a építési projektek részletező és kockázatelemző panel hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a építési projektek részletező és kockázatelemző panel hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a építési projektek részletező és kockázatelemző panel hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a építési projektek részletező és kockázatelemző panel hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a építési projektek részletező és kockázatelemző panel hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: `GET /api/v1/planning/baugesuche?postcode={PLZ}&active_only={bool}`
