---
id: FEAT-037
title: Mikroklíma, Városi Hősziget és Klímajövő
status: SPEC_READY
version: 1
risk: high
owner: product-owner
...
# FEAT-037: Mikroklíma, Városi Hősziget és Klímajövő
...
- Kapcsolódó brief: `BRIEF-037` (`docs/briefs/BRIEF-037-mikroklima-hosziget-es-klimajovo.md`)
...
- REQ-001 [MUST]: A rendszer a mikroklíma, városi hősziget és klímajövő hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a mikroklíma, városi hősziget és klímajövő hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a mikroklíma, városi hősziget és klímajövő hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a mikroklíma, városi hősziget és klímajövő hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a mikroklíma, városi hősziget és klímajövő hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/climate/profile?lat={lat}&lon={lon}&horizon={enum}`
