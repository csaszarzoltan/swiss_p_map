---
id: FEAT-024
title: Megosztható Mélylink és Nyelvperzisztencia
status: SPEC_READY
version: 1
risk: low
owner: product-owner
...
# FEAT-024: Megosztható Mélylink és Nyelvperzisztencia
...
- Kapcsolódó brief: `BRIEF-024` (`docs/briefs/BRIEF-024-megoszthato-melylink-es-allapot.md`)
- Kapcsolódó research: `docs/research/2026-08-28-usability-deep-dive.md`
- Kapcsolódó ADR: `ADR-022` (`docs/decisions/ADR-022-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `frontend/src/hooks/useShareableState.ts; frontend/src/components/ShareButton.tsx`
...
- REQ-001 [MUST]: A rendszer a megosztható mélylink és nyelvperzisztencia hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a megosztható mélylink és nyelvperzisztencia hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a megosztható mélylink és nyelvperzisztencia hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a megosztható mélylink és nyelvperzisztencia hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a megosztható mélylink és nyelvperzisztencia hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: Nincs backend végpont; URL-séma: `/{locale}?plz=NNNN&topic={enum}&selected={id}&radius={m}`.
