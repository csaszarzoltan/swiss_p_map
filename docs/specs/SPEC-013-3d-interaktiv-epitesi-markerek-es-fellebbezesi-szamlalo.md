...
- Kapcsolódó brief: `BRIEF-013` (`docs/briefs/BRIEF-013-3d-interaktiv-epitesi-markerek-es-fellebbezesi-szamlalo.md`)
- Kapcsolódó research: `docs/research/2026-08-27-3d-baugesuch-pins.md`
- Kapcsolódó ADR: `ADR-013` (`docs/decisions/ADR-013-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `frontend/src/app/Map3D.tsx; src/services/ogd_service.py`
...
- REQ-001 [MUST]: A rendszer a 3d interaktív építési markerek és fellebbezési számláló hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a 3d interaktív építési markerek és fellebbezési számláló hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a 3d interaktív építési markerek és fellebbezési számláló hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a 3d interaktív építési markerek és fellebbezési számláló hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a 3d interaktív építési markerek és fellebbezési számláló hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: `GET /api/v1/planning/baugesuche?postcode={PLZ}&active_only=true`
