---
id: FEAT-040
title: Levegőminőség, Pollen és Környezeti Terhelés
status: SPEC_READY
version: 1
risk: high
owner: product-owner
...
# FEAT-040: Levegőminőség, Pollen és Környezeti Terhelés
...
- Kapcsolódó brief: `BRIEF-040` (`docs/briefs/BRIEF-040-levego-minoseg-pollen-es-kornyezeti-terheles.md`)
...
- REQ-001 [MUST]: A rendszer a levegőminőség, pollen és környezeti terhelés hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a levegőminőség, pollen és környezeti terhelés hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a levegőminőség, pollen és környezeti terhelés hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a levegőminőség, pollen és környezeti terhelés hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a levegőminőség, pollen és környezeti terhelés hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/environment/air?lat={lat}&lon={lon}`; `GET /api/v1/environment/pollen?region={id}`
