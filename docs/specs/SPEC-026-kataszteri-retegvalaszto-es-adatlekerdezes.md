---
id: FEAT-026
title: Kataszteri Rétegválasztó és Parcella-adatlekérdezés
status: SPEC_READY
version: 1
risk: high
owner: product-owner
...
# FEAT-026: Kataszteri Rétegválasztó és Parcella-adatlekérdezés
...
- Kapcsolódó brief: `BRIEF-026` (`docs/briefs/BRIEF-026-kataszteri-retegvalaszto-es-adatlekerdezes.md`)
...
- REQ-001 [MUST]: A rendszer a kataszteri rétegválasztó és parcella-adatlekérdezés hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a kataszteri rétegválasztó és parcella-adatlekérdezés hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a kataszteri rétegválasztó és parcella-adatlekérdezés hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a kataszteri rétegválasztó és parcella-adatlekérdezés hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a kataszteri rétegválasztó és parcella-adatlekérdezés hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: `GET /api/v1/cadastre/layers?canton={code}`; `POST /api/v1/cadastre/identify`
