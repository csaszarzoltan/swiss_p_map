...
brief: BRIEF-058
---
# SPEC-058: Kantonális E-Amtsblatt és Hivatalos Közlöny Hírfolyam Aggregátor
## 1. Cím és metaadatok
`SPEC-058`, `BRIEF-058`, verzió 1, SPEC_READY, implementáció PENDING_DEV.
## 2. Cél és kontextus
Resident-first civic UX, élő OGD integráció vagy hozzájárulás-alapú értesítés biztosítása ellenőrizhető forrással.
## 3. Funkcionális követelmények
- REQ-058-001 [MUST]: A feature típusos, determinisztikus sikeres választ vagy UI-eredményt ad.
- REQ-058-002 [MUST]: Minden külső adat source, fetched_at és trust_state metaadatot tartalmaz.
- REQ-058-003 [MUST]: Loading, success, empty, stale/source_pending és error állapot támogatott.
- REQ-058-004 [MUST NOT]: Duplikált értesítés, hamis élő adat vagy hozzájárulás nélküli kézbesítés tilos.
- REQ-058-005 [ALWAYS]: A fallback nem jelenhet meg official_measurement státusszal.
- REQ-058-006 [CONCURRENCY]: Idempotency/deduplikáció és legutóbbi kérés nyer szabály kötelező.
## 4. Nem-funkcionális követelmények
- NFR-058-001 [PERFORMANCE]: UI feedback 100 ms; API timeout 10 s; connector cache a provider ciklusához kötött.
- NFR-058-002 [ACCESSIBILITY]: WCAG 2.1 AA, billentyűzet, aria-live, grafikonhoz szöveges alternatíva.
- NFR-058-003 [PRIVACY]: nDSG/GDPR minimalizálás; email/push token titkos vagy hash-elt tárolása.
## 5. Elfogadási kritériumok
### AC-058-001: Happy path
...
### AC-058-003: Hibás vagy ismételt kérés
...
`POST /api/v1/connectors/amtsblatt/ingest; GET /api/v1/news/local`. JSON response source/trust/cache metaadatokkal; írások idempotency mezővel.
...
## 11. Nemzetköziesítés
DE/EN/FR/IT kulcsnévtér: `phase3.feature058.*`.
## 12. Akadálymentesség
...
- REQ-058-001 -> AC-058-001 -> unit + API/E2E.
- REQ-058-002 -> AC-058-001 -> contract test.
- REQ-058-003 -> AC-058-001..003 -> UI state test.
- REQ-058-004 -> AC-058-003 -> negative/dedup test.
- REQ-058-005 -> AC-058-002 -> fallback test.
- REQ-058-006 -> AC-058-003 -> concurrency/idempotency test.
