...
- Kapcsolódó brief: `BRIEF-030` (`docs/briefs/BRIEF-030-mobil-akadalymentes-es-billentyuzetes-hasznalat.md`)
...
- REQ-001 [MUST]: A rendszer a mobil, akadálymentes és billentyűzetes használat hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a mobil, akadálymentes és billentyűzetes használat hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a mobil, akadálymentes és billentyűzetes használat hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a mobil, akadálymentes és billentyűzetes használat hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a mobil, akadálymentes és billentyűzetes használat hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Tervezett szerződés: Nincs új domain API; hozzáférhetőségi és viewport szerződés minden meglévő felületre.
