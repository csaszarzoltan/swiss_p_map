---
id: FEAT-003
title: 3D Sematikus Svájci Térkép és Kantonális Navigáció
status: SPEC_READY
version: 1
risk: low
owner: product-owner
...
# FEAT-003: 3D Sematikus Svájci Térkép és Kantonális Navigáció
...
- Kapcsolódó brief: `BRIEF-003` (`docs/briefs/BRIEF-003-3d-sematikus-svajci-terkep-es-kantonalis-navigacio.md`)
- Kapcsolódó research: `docs/research/svajc_3d_terkep.html`
- Kapcsolódó ADR: `ADR-003` (`docs/decisions/ADR-003-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `frontend/src/app/Map3D.tsx; frontend/src/app/swissCantons.ts`
...
- REQ-001 [MUST]: A rendszer a 3d sematikus svájci térkép és kantonális navigáció hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a 3d sematikus svájci térkép és kantonális navigáció hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a 3d sematikus svájci térkép és kantonális navigáció hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a 3d sematikus svájci térkép és kantonális navigáció hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a 3d sematikus svájci térkép és kantonális navigáció hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: Nincs önálló backend végpont; a kliens a meglévő Place, Politics és Planning API-kból renderel.
