---
id: FEAT-023
title: Térképi Sugár-Figyelő és Közelben Lévő Projektek
status: SPEC_READY
version: 1
risk: low
owner: product-owner
...
# FEAT-023: Térképi Sugár-Figyelő és Közelben Lévő Projektek
...
- Kapcsolódó brief: `BRIEF-023` (`docs/briefs/BRIEF-023-terkepi-sugar-figyelo.md`)
- Kapcsolódó research: `docs/research/2026-08-28-usability-deep-dive.md`
- Kapcsolódó ADR: `ADR-021` (`docs/decisions/ADR-021-*.md`)
...
- Implementációs bizonyíték / tervezett érintettség: `frontend/src/components/WatchZone.tsx; frontend/src/app/Map3D.tsx`
...
- REQ-001 [MUST]: A rendszer a térképi sugár-figyelő és közelben lévő projektek hiányos vagy hibás adatát explicit állapotként jelzi. A művelet sikerét a felületen és az API-válaszban azonos domainállapot reprezentálja.
- REQ-002 [MUST]: A rendszer a térképi sugár-figyelő és közelben lévő projektek hiányos vagy hibás adatát explicit állapotként jelzi. Minden megjelenített értékhez típus, mértékegység és forrás tartozik, ha az érték adatforrásból ered.
- REQ-003 [MUST]: A rendszer a térképi sugár-figyelő és közelben lévő projektek hiányos vagy hibás adatát explicit állapotként jelzi. Hiányzó, részleges vagy alacsony felbontású adatot a rendszer explicit `unavailable`, `estimated` vagy `fallback` állapottal jelöl.
- REQ-004 [MUST]: A rendszer a térképi sugár-figyelő és közelben lévő projektek hiányos vagy hibás adatát explicit állapotként jelzi. Az elsődleges művelet egér, érintés és billentyűzet használatával is végrehajtható.
...
When a rendszer a térképi sugár-figyelő és közelben lévő projektek hiányos vagy hibás adatát explicit állapotként jelzi
...
## 11. API-szerződés
- Szerződés: `GET /api/v1/planning/radius?lat={lat}&lon={lon}&radius_m={300|500|1000|2000}&active_only=true`
