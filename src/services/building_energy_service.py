...
class BuildingEnergyService:
    def assess(self, postcode: str) -> BuildingEnergyAssessment:
        ...
        return BuildingEnergyAssessment(
            postcode=postcode,
            dominant_heating="district_heating" if urban else "heat_pump",
            district_heating_potential="high" if urban else "medium",
            geothermal_potential="medium",
            checklist=[
                "Gebäudehülle und Heizbedarf prüfen",
                "Wärmepumpe oder Fernwärme abklären",
                "Kantonale Förderung vor Bestellung prüfen",
            ],
        )
