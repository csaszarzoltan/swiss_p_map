...
GATEWAY_URL = os.environ.get("SWISSPM_AI_GATEWAY_URL", "http://127.0.0.1:8013/v1/chat/completions")
SYSTEM_TMPL = {
...
def build_prompt(locale: str, postcode: str, place: dict[str, object], politics: dict[str, object], baugesuche: list[dict[str, object]]) -> tuple[str, str]:
    loc = locale if locale in SYSTEM_TMPL else "de"
    system = SYSTEM_TMPL[loc]
    payload = json.dumps({"postcode": postcode, "place": place, "politics": politics, "baugesuche": baugesuche[:4]}, ensure_ascii=False)
...
class AiSummaryService:
    ...
    def __init__(self, client: httpx.AsyncClient | None = None, gateway_url: str | None = None) -> None:
        self._client = client
        self._url = gateway_url or GATEWAY_URL

    async def summarize(self, locale: str, postcode: str, place: dict[str, object], politics: dict[str, object], baugesuche: list[dict[str, object]]) -> str | None:
        ...
        try:
            if self._client is not None:
                resp = await self._client.post(self._url, json=body, headers={"Content-Type": "application/json"})
            else:
                async with httpx.AsyncClient(timeout=12) as c:
                    resp = await c.post(self._url, json=body, headers={"Content-Type": "application/json"})
