...
class HealthcareService:
    def access(self, postcode: str) -> HealthcareAssessment:
        data: list[tuple[FacilityType, str, int, int]] = [
            ("pharmacy", "Nächste Apotheke", 4, 7),
            ("urgent_care", "Notfallpraxis", 9, 15),
            ("hospital", "Regionalspital", 14, 22),
        ]
        return HealthcareAssessment(
            ...
            facilities=[
                HealthcareFacility(
                    ...
                    name=n,
                    car_time_min=c,
                    public_transport_time_min=p,
                    official_url="https://www.bag.admin.ch/",
                )
                for t, n, c, p in data
            ],
        )
