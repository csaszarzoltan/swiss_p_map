...
class CantonVoteResult(BaseModel):
    ...
    canton_name: str = Field(..., description="Official canton name (e.g. Zürich, Bern)")
    yes_percent: float = Field(..., ge=0.0, le=100.0, description="Percentage of YES votes")
    no_percent: float = Field(..., ge=0.0, le=100.0, description="Percentage of NO votes")
    turnout_percent: float = Field(..., ge=0.0, le=100.0, description="Voter turnout percentage")
...
class FederalVoteProposal(BaseModel):
    ...
    titles: dict[str, str] = Field(..., description="Multilingual titles (de, en, fr, it)")
